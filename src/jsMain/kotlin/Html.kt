...
internal typealias HtmlBuilder = TagConsumer<HTMLElement>

public interface Html : HtmlBuilder

public abstract class InternalHtml(internal val parent: HtmlBuilder) : Html, HtmlBuilder by parent

public class MyJSDOMBuilder(public val document: Document) : TagConsumer<HTMLElement> {
    ...
    override fun onTagStart(tag: Tag) {
        ...
        if (tag is Test) {
            ...
            println("test tag found -> $last")
            return
        }
        // pass parent component
        // mark as rendered
        // unmount ones not in a list when rendering is done !!!!
    ...
    override fun onTagEvent(tag: Tag, event: String, value: (Event) -> Unit) {
        when {
            path.isEmpty() -> throw IllegalStateException("No current tag")
            path.last().tagName.lowercase() != tag.tagName.lowercase() -> throw IllegalStateException("Wrong current tag")
            else -> path.last().also { asDynamic()[event] = value }
        }
    }

    override fun onTagEnd(tag: Tag) {
        if (tag is Test) return
